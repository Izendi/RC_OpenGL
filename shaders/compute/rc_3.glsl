...
const float N_plus_1_ProbeStartOffset = 16.0;

const float N_plus_1_ProbeSpacing = 32.0;

const float N_plus_1_MaxNumberOfProbes = 64;

const float N_Offset = 8.0;

const float N_ProbeSpacing = 16.0;
...
vec4 averageColorValuesAtNplus1_glInvocation(uvec3 g_TLP, uvec3 g_TRP, uvec3 g_BLP, uvec3 g_BRP)
{
    ...
    vec4 color_0 = texelFetch(u_tex_rc4, texelCoord_0, 0);
    vec4 color_1 = texelFetch(u_tex_rc4, texelCoord_1, 0);
    vec4 color_2 = texelFetch(u_tex_rc4, texelCoord_2, 0);
    vec4 color_3 = texelFetch(u_tex_rc4, texelCoord_3, 0);
...
vec4 get_N_plus_1_4RayProbeAveragedColorValue(uint thisIterationID)
{
    ...
    uvec3 workGroupSize_xyz = uvec3(32, 32, 1);
    ...
    float iterID_0_x = iterID_0 - (iterID_0_y * 32);
    ...
    float fx = (float((gl_WorkGroupID.x * 16) + 8) - float((TLP_workGroupID_xyz.x * 32 + 16))) / 32;
    float fy = (float((gl_WorkGroupID.y * 16) + 8) - float((TLP_workGroupID_xyz.y * 32 + 16))) / 32;
