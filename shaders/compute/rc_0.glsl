...
uniform sampler2D u_tex_rc1;

const float N_plus_1_ProbeStartOffset = 2.0;

const float N_plus_1_ProbeSpacing = 4.0;

const float N_plus_1_MaxNumberOfProbes = 128; //In one dimension

const float N_Offset = 1.0;

const float N_ProbeSpacing = 2.0;
...
vec4 averageColorValuesAtNplus1_glInvocation(uvec3 g_TLP, uvec3 g_TRP, uvec3 g_BLP, uvec3 g_BRP)
{
    ...
    vec4 color_0 = texelFetch(u_tex_rc1, texelCoord_0, 0);
    vec4 color_1 = texelFetch(u_tex_rc1, texelCoord_1, 0);
    vec4 color_2 = texelFetch(u_tex_rc1, texelCoord_2, 0);
    vec4 color_3 = texelFetch(u_tex_rc1, texelCoord_3, 0);

    vec4 avgColor = ((color_0 + color_1 + color_2 + color_3) / 4.0);// / 4.0;
    avgColor.a = 1.0;
...
vec4 get_N_plus_1_4RayProbeAveragedColorValue(uint thisIterationID)
{
    ...
    uvec3 workGroupSize_xyz = uvec3(4, 4, 1);
    ...
    float iterID_0_x = iterID_0 - (iterID_0_y * 4);
    ...
    float fx = (float((gl_WorkGroupID.x * 2) + 1) - float((TLP_workGroupID_xyz.x * 4 + 2))) / 4;
    float fy = (float((gl_WorkGroupID.y * 2) + 1) - float((TLP_workGroupID_xyz.y * 4 + 2))) / 4;
...
void main()
{
    ...
    float angle = radians((45.0 + (90.0 * iterationID)));
    vec2 dirVec = vec2(cos(angle), sin(angle)); //This vector is already normalized by default.

    ivec2 texelCoord = ivec2(gl_GlobalInvocationID.xy);

    float rayStartDistanceFromNode = 0.0;
    float xyIntervalsPerWorkgroup = 2.0;
    float xyOffsetPerWorkgroup = 1.0;
