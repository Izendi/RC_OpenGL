...
void main()
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        if (totalRayTravelDistance > lvl_4_interval)
        {
            break;
        }
		...
		vec2 vecToTravel = dirVec * distanceFromNearestSDF;

        ray = ray + vecToTravel;

        totalRayTravelDistance = totalRayTravelDistance + length(vecToTravel);
